  ...
  //Scatterplot 1
  {
    ...
    svg1
        .call( d3.brush()                 // Add the brush feature using the d3.brush function
          .extent( [ [0,0], [460,450] ] ) // initialise the brush area: start at 0,0 and finishes at width,height: it means I select the whole graph area
          .on("start brush", updateChart1)
         // .on("end brush", clear1) // Each time the brush selection changes, trigger the 'updateChart' function
        )
  ...
  //TODO: Scatterplot 2 (show Sepal width on x-axis and Petal width on y-axis)
  {
    ...
    svg2
        .call( d3.brush()                 // Add the brush feature using the d3.brush function
          ...
          //.on("end brush", clear2) // Each time the brush selection changes, trigger the 'updateChart' function
        )
...

{
  ...
  //Removes existing brushes from svg
    function clear1() {
        svg1.call(brush.move, null);
    }

    //Removes existing brushes from svg
    function clear2() {
        svg2.call(brush.move, null);
    }
    ...
    function updateChart2(brushEvent) {
      extent = brushEvent.selection;
      var selectedSpecies = svg2.selectAll(".selected").nodes().map((circle) => circle['species']);
      myCircle1.classed("selected", (d) => {
          return isBrushed(extent, x2(d[xKey2]), y2(d[yKey2]))
        })
      svg3.selectAll("rect").classed("selected", (d) => {
